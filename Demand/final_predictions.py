...
for j in range(1):
    ...
    for i in range(int(input("Range: "))):
        ...
        print('>Predicted %d = %.3f' % (i,trend_pre))
    # errors
    ...
    pyplot.plot(data,label='Electricity Consumption')
    pyplot.plot([None for x in range(12)]+[x for x in train_trend],color='red',label='Trend')
    pyplot.plot([None for x in range(12+len(train_trend))]+[train_trend[(len(train_trend)-1)]]+[x for x in predictions_trend],color='red',label='Predicted Trend')
    pyplot.plot([None for x in range(12+len(train_trend))]+[train_trend[(len(train_trend)-1)]]+[x for x in predictions_trend_plus_seasonality],color='green',label='Electricity Consumption Predictions')
    pyplot.show()
